...
"""
channel_dict = {2412: 1, 2417: 2, 2422: 3, 2427: 4, 2432: 5, 2437: 6, 2442: 7, 2447: 8, 2452: 9, 2457: 10, 2462: 11, 2467: 12, 2472: 13, 2484: 14}


def get_channel(pkt, present_flag_list, base_length):
    channel_location = base_length + present_flag_list[0] * 8 + present_flag_list[1] * 1 + present_flag_list[2] * 1
    channel = channel_dict[int.from_bytes(pkt[channel_location:channel_location+2], byteorder='little')]
...
def get_is_qos_data(pkt, radiotap_length):
    is_qos_data = (pkt[radiotap_length] & 0b10000000) >> 7

    return {0: False, 1:True}.get(is_qos_data, False)
...
class Dot11:
    def __init__(self, pkt):
        ...
        # self.channel = get_channel(pkt, self.present_flag_list, self.base_length)
        self.pwr = get_pwr(pkt, self.present_flag_list, self.base_length)

        # mac info
        self.mac_header = self.radiotap_length

        self.type = get_type((pkt[self.mac_header] & 0b00001100) >> 2)

        if self.type == "Management":
            self.sub_type = get_mgt_sub_type((pkt[self.radiotap_length] & 0b11110000) >> 4)
            ...
            if self.sub_type == "Beacon" or self.sub_type == "ProbeResp":
                ...
                self.mb = str(self.max_rate) + self.preamble
            ...
            self.is_qos_data = get_is_qos_data(pkt, self.radiotap_length)
